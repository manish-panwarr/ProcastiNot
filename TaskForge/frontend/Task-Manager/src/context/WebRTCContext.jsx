...
//@desc : Default reliable STUN servers — used immediately while /api/ice-servers loads
...
export const WebRTCProvider = ({ children }) => {
    ...
    //@desc : Holds the latest ICE server list — fetched from backend so TURN credentials are never exposed in the frontend bundle.
    ...
    //@desc : Fetch ICE servers from backend once on mount.
    useEffect(() => {
        const fetchIceServers = async () => {
            try {
                const res = await fetch(`${BASE_URL}/api/ice-servers`);
                if (res.ok) {
                    const servers = await res.json();
                    if (Array.isArray(servers) && servers.length > 0) {
                        iceServersRef.current = servers;
                        console.log("[ICE] Loaded", servers.length, "ICE server(s) from backend",
                            servers.some(s => s.urls?.toString().startsWith("turn")) ? "(TURN included)" : "(STUN only — set TURN env vars on Render)"
                        );
                    }
                }
            } catch (e) {
                console.warn("[ICE] Could not fetch ICE servers from backend — using STUN-only fallback:", e.message);
            }
        };
        fetchIceServers();
    }, []);

    //@desc : Add to local state
    ...
    //@desc : Force-destroy a peer (safe to call even if already destroyed)
    ...
    //@desc : Create (or return existing live) peer
    const createPeer = useCallback((targetId, isInitiator) => {
        const id = String(targetId);

        //@desc : If existing peer is alive and connected, return it
        ...
        //@desc : Production ICE debugging & aggressive failure handling 
        const rawPc = peer._pc;
        if (rawPc) {
            rawPc.addEventListener("connectionstatechange", () => {
                console.log(`[P2P] ${id} connectionState →`, rawPc.connectionState);
                if (rawPc.connectionState === "failed") {
                    console.error(`[P2P] ICE failed for ${id}. Strict NAT detected without relay support.`);
                    peer.destroy(new Error("Network firewalls blocked the direct connection (TURN proxy required)."));
                }
            });
            rawPc.addEventListener("icegatheringstatechange", () => {
                console.log(`[P2P] ${id} iceGatheringState →`, rawPc.iceGatheringState);
            });
        }
        ...
        //@desc : Receive data with chunk reassembly
    ...
    //@desc : Handle incoming WebRTC signals from the server
    useEffect(() => {
        ...
        const onSignal = ({ signal, from }) => {
            ...
            try {
                ...
                console.warn('p2p signal warning (ignoring safely):', e.message);
            }
        };
    ...
    //@desc : Send P2P text message or file
